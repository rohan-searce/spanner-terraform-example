...
variable "ddl_queries" {
  ...
  default = [
    ...
    "CREATE UNIQUE NULL_FILTERED INDEX companiesByCompanyNameShortCode ON companies (companyName, companyShortCode)",
    "CREATE TABLE companyStocks (companyId STRING(36) NOT NULL, companyStockId STRING(36) NOT NULL, open NUMERIC, volume NUMERIC, currentValue NUMERIC, date FLOAT64, close NUMERIC, dayHigh NUMERIC, dayLow NUMERIC, timestamp TIMESTAMP NOT NULL OPTIONS (allow_commit_timestamp = true), CONSTRAINT FK_CompanyStocks FOREIGN KEY (companyId) REFERENCES companies (companyId)) PRIMARY KEY(companyId, companyStockId), INTERLEAVE IN PARENT companies ON DELETE CASCADE",
    "CREATE TABLE simulations (companyId STRING(36) NOT NULL, sId STRING(36) NOT NULL, status STRING(36), createdAt TIMESTAMP NOT NULL OPTIONS (allow_commit_timestamp = true), CONSTRAINT FK_CompanySimulation FOREIGN KEY (companyId) REFERENCES companies (companyId) ) PRIMARY KEY(companyId, sId), INTERLEAVE IN PARENT companies ON DELETE CASCADE",
  ]
