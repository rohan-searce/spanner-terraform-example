...
variable "ddl_queries" {
  ...
  default = [
    ...
    "CREATE TABLE companyStocks (companyId STRING(36) NOT NULL, companyStockId STRING(36) NOT NULL, companyShortCode STRING(36), open NUMERIC, volume NUMERIC, currentValue NUMERIC, date FLOAT64, close NUMERIC, dayHigh NUMERIC, dayLow NUMERIC, timestamp TIMESTAMP NOT NULL OPTIONS (allow_commit_timestamp = true), CONSTRAINT FK_CompanyStocks FOREIGN KEY (companyId) REFERENCES companies (companyId)) PRIMARY KEY(companyId, companyStockId), INTERLEAVE IN PARENT companies ON DELETE CASCADE",
